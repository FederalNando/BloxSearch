...
const get = async (url, attempts = 0) => {
  try {
    ...
    await sleep(1);
    return await get(url, attempts + 1);
  }
};

const post = async (url, body, attempts = 0) => {
  try {
    const request = await fetch(`https://${url}`, {
      method: 'POST',
      body,
    ...
    await sleep(1);
    return await post(url, body, attempts + 1);
  }
};
...
async function findTarget(imageUrl, place) {
  ...
  if (targetServersId.length) {
    targetServersId.forEach((targetServerId) => {
      ...
      status.innerText = `Found target: ${targetServerId}`;
    });
  } else {
...
search.addEventListener('click', async event => {
  ...
  const user = await post('users.roblox.com/v1/usernames/users', JSON.stringify({usernames: [`${input.value}`]}))
  ...
  const { data: [{ imageUrl }] } = await get(`thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${user.data[0].id}&size=150x150&format=Png&isCircular=false`);
